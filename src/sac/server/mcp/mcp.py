...
@mcp.tool(
    ...
    description=(
        ...
        "IMPORTANT: Do NOT include CSS/styling directions (e.g. 'use dark theme', "
        "color schemes) — SaC controls visual design autonomously. Only include "
        "styling if the user explicitly requested it.\n\n"
        ...
        "resume, just ask you to listen again.\n\n"
        "TIMEOUT RECOVERY: If generate_app times out, the server is likely "
        "still generating — the viewer will show the result when ready. "
        "Do NOT retry generate_app. Instead:\n"
        "1. Call `list_conversations` to find the latest conversation\n"
        "2. Show the user the viewer URL: http://127.0.0.1:18420/c/{conversation_id}\n"
        "3. Call `wait_for_action` with that conversation_id to continue the loop\n"
        "The user can already see live progress streaming in the viewer."
    ),
)
...
@mcp.tool(
    ...
    description=(
        ...
        "IMPORTANT: SaC's LLM already has the full current app code as context. "
        "You do NOT need to re-describe the entire app — just concisely describe the "
        "change or additional content you want. Avoid repeating information.\n\n"
        "Do NOT include CSS/styling directions — SaC controls visual design "
        "autonomously. Only include styling if the user explicitly requested it.\n\n"
        "TIMEOUT RECOVERY: If evolve_app times out, the server is likely "
        "still generating — the viewer will show the result when ready. "
        "Do NOT retry evolve_app (that creates a duplicate version). Instead, "
        "call `wait_for_action` to continue the loop."
    ),
)
