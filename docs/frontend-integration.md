...
```bash
python -m sac.cli serve --port 18420
```

The server runs at `http://localhost:18420` with CORS enabled for all origins.
...
function useGenerate() {
  ...
  const generate = useCallback((intent: string) => {
    ...
    const es = new EventSource(`http://localhost:18420/stream?${params}`);
...
```js
const res = await fetch('http://localhost:18420/generate', {
  method: 'POST',
...
```js
const res = await fetch('http://localhost:18420/evolve', {
  method: 'POST',
  headers: { 'Content-Type': 'application/json' },
  body: JSON.stringify({
...
<script type="module">
import { SaCRenderer } from 'http://localhost:18420/renderer/sac-renderer.js';

const renderer = new SaCRenderer(document.getElementById('preview'), {
  previewUrl: 'http://localhost:18420/renderer/preview.html',
  designSystem: 'http://localhost:18420/renderer/design-systems/default/shim.js',
});
...
```js
const { conversations } = await fetch('http://localhost:18420/conversations').then(r => r.json());
...
```js
const { conversation, events } = await fetch(`http://localhost:18420/conversations/${id}`).then(r => r.json());
